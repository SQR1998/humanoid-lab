...
└── add-linglong-robot
    └── add-linglong-locomotion-task
        └── add-linglong-standing-mode
            └── add-linglong-static-stand-rewards
```
...
### 4. `add-linglong-standing-mode`

该分支在稳定行走任务基础上增加真正的静态站立训练，同时保留原来的灵龙行走任务。

主要修改：

- 新增任务注册：`Locomotion-Flat-LingLong-30Dof-Stand-v0`。
- 原任务 `Locomotion-Flat-LingLong-30Dof-v0` 保持可用。
- 每次重新采样指令时，20% 的环境进入静态站立模式。
- 静态站立模式将三维速度指令全部设为零。
- 静态站立模式停止交替抬脚参考，要求双脚同时接触地面。
- 静态站立模式使用机器人默认站姿作为腿部关节参考。

验证计划：

- 先进行 64 环境、2 轮 PPO 烟雾测试。
- 烟雾测试通过后，使用 RTX 4090、16384 环境训练 5000 轮。
- 可视化时分别检查零速度站立、直行、转向和侧向行走。

注意：

- 该任务需要重新训练；旧行走模型没有学习过静态站立样本。
- 站立环境比例固定为 20%，其余 80% 继续学习随机速度行走。

### 5. `add-linglong-static-stand-rewards`

该分支在静态站立版本基础上，修复零速度时仍然交替抬腿的问题，并保持策略网络输入维度与 PPO 参数不变。

主要修改：

- 保留 20% 静态站立、80% 随机速度行走的训练比例。
- 新增仅在静态站立样本生效的腿部关节位置约束。
- 新增仅在静态站立样本生效的腿部关节速度惩罚。
- 新增仅在静态站立样本生效的双脚线速度惩罚。
- 新增仅在静态站立样本生效的脚部离地惩罚。
- 正常行走样本中以上四项自动返回零，不直接限制直行、侧移和转向。
- Actor 和 Critic 的输入输出维度不变，可从站立版本的 `model_4999.pt` 继续训练。

建议验证顺序：

1. 从已有 5000 轮模型恢复，先进行 64 环境、2 轮烟雾测试。
2. 烟雾测试通过后，使用 16384 环境继续训练 500 到 1000 轮。
3. 先强制静态站立检查是否停止交替抬腿，再检查直行、侧移和转向是否退化。
...
现阶段在 `add-linglong-static-stand-rewards` 分支，从已完成的 5000 轮站立模型继续训练，重点消除零速度时的交替抬腿，同时复查直行、侧移和转向是否保持正常。
